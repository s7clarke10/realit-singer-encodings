...
class Connection:
    def get_files(self, search_prefix, search_pattern):
        return [
            {"last_modified": "2020-01-01", "filepath": "root_dir/file1.csv"},
            {"last_modified": "2020-01-02", "filepath": "root_dir/file2.csv"},
        ]
...
class TestEmptySchema(unittest.TestCase):
    def test_not_empty_samples(self, mocked_sample_files):
        ...
        conn = Connection()
        schema = json_schema.get_schema_for_table(
            conn,
            {
                "table_name": "data",
                "search_prefix": "/root_dir",
