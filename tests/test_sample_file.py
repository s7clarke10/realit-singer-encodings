...
from singer_encodings import csv, json_schema
...
class TestSampleFile(unittest.TestCase):
    def test_positive(self, mocked_csv_row_iterator):
        mocked_csv_row_iterator.return_value = [
            _csv.DictReader(io.StringIO("header\nvalue"))
        ]
        conn = Connection()
        empty_file, samples = json_schema.sample_file(
            conn,
            {"table_name": "data", "key_properties": ["id"], "delimiter": ","},
            {
        ...
        # check if "csv.get_row_iterators" is called if it is called then error
        # has not occurred if it is not called then error has occured and
        # function returned from the except block
