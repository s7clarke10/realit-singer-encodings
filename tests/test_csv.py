...
class TestOptionsWithDuplicateHeaders(unittest.TestCase):
    ...
    def test(self):
        ...
        row_iterator = csv.get_row_iterator(
            self.csv_data,
            options={"date_overrides": ["columnA"]},
            headers_in_catalog=None,
            with_duplicate_headers=True,
        )
        rows = [r for r in row_iterator]
        self.assertEqual(rows[0]["columnA"], "1")
        ...
        try:
            ...
            expected_message = (
                "CSV file missing date_overrides headers: {'fizz'}"
            )
            self.assertEqual(expected_message, str(ex))
...
class TestDuplicateHeaders(unittest.TestCase):
    ...
    csv_data = [
        b"columnA,columnB,columnC,columnB,columnC,columnC",
        b"1,2,3,4,5,6",
    ]
    ...
    def test(self):
        ...
        rows = [r for r in row_iterator]
        self.assertEqual(
            rows[0]["_sdc_extra"], [{"columnB": "4"}, {"columnC": ["5", "6"]}]
        )
        self.assertEqual(
            list(rows[0].keys()),
            ["columnA", "columnB", "columnC", "_sdc_extra"],
        )
...
class TestDuplicateHeadersRestValue(unittest.TestCase):
    ...
    def test(self):
        ...
        rows = [r for r in row_iterator]
        self.assertEqual(
            rows[0]["_sdc_extra"], [{"columnB": "4"}, {"columnC": "5"}]
        )
        self.assertEqual(
            list(rows[0].keys()),
            ["columnA", "columnB", "columnC", "_sdc_extra"],
...
class TestDuplicateHeadersRestValueNoSDCExtra(unittest.TestCase):
    ...
    def test(self):
        ...
        rows = [r for r in row_iterator]
        self.assertEqual(
            list(rows[0].keys()), ["columnA", "columnB", "columnC"]
        )
...
class TestMissingFiedInCatalog(unittest.TestCase):
    ...
    def test(self):
        row_iterator = csv.get_row_iterator(
            self.csv_data, None, ["columnA", "columnB"], True
        )
        rows = [r for r in row_iterator]
        self.assertListEqual(
            rows[0]["_sdc_extra"],
            [{"columnC": ["3", "5", "6"]}, {"columnB": "4"}],
        )
        self.assertEqual(
            list(rows[0].keys()), ["columnA", "columnB", "_sdc_extra"]
        )
...
class TestWarningForDupHeaders(unittest.TestCase):
    ...
    @mock.patch("singer_encodings.csv_helper.LOGGER.warning")
    def test(self, mocked_logger_warn):
        ...
        mocked_logger_warn.assert_called_with(
            'Duplicate Header(s) %s found in the csv and its value will be stored in the "_sdc_extra" field.',  # noqa: E501
            {"columnC"},
        )
...
class TestReturnNoneForEmptyCSV(unittest.TestCase):
    ...
    def test_get_row_iterator_return_none_for_empty_csv(
        self, mocked_logger_warn
    ):
